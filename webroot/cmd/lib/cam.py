...
#def start():
    c = conf()
    data = {"offsetY":      c.get('cam_offsety'),
            "offsetX":      c.get('cam_offsetx'),
...
def connectAndStartCam(data):
    ...
    connection = urlopen(url, timeout=float(c.get('g_timeout')))
    return connection
...
def createStartMessage(c):
    ...
    message += "X(" + c.get('cam_offsetx')    + ")"
    message += "Y(" + c.get('cam_offsety')    + ")"
